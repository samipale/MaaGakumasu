...
class ProduceChooseEventAuto(CustomAction):
    ...
    # 阈值常量
    GO_OUT_HEALTH_THRESHOLD = 0.6
    LOW_HEALTH_THRESHOLD = 0.34
    HIGH_POINT_THRESHOLD = 200
    CLICK_DELAY = 0.5
    ACTION_DELAY = 3.0
    ...
    def run(self, context: Context, argv: CustomAction.RunArg) -> bool:
        ...
        # 合并1~2的流程，处理多个SP课程时的最优解
        if self._handle_course(context, image):
            return True
    ...
    def _handle_course(self, context: Context, image) -> bool:
        """合并老师建议与sp课程，综合处理课程选择"""
        ...
        if reco_detail.hit:
            logger.info("存在SP课程")
            sp_course = reco_detail.filtered_results
            sp_best_course = reco_detail.best_result.box
        else:
            sp_course = None
            sp_best_course = [0, 0]
        ...
        if not reco_detail.hit and not sp_course:
            return False
        ...
        if "rest" in suggestion_img:
            ...
            if (go_out_box and health_ratio
                    and health_ratio > max(self.LOW_HEALTH_THRESHOLD, self.GO_OUT_HEALTH_THRESHOLD)):
                logger.info("选择外出")
        ...
        # 其他建议课程
        if sp_course is None:
            self._double_click(context, result[0] + 80, result[1] + 80)
            time.sleep(self.ACTION_DELAY)
            return True
        if len(sp_course) == 1:
            self._double_click(context, sp_best_course[0] + 80, sp_best_course[1] + 80)
            time.sleep(self.ACTION_DELAY)
            return True
        # 多个sp课程时，优先选择老师建议的
        for spc in sp_course:
            if abs(result[0] - spc.box[0]) < 40:
                self._double_click(context, spc.box[0] + 80, spc.box[1] + 80)
                time.sleep(self.ACTION_DELAY)
                return True
        # 如果sp课程里面没有老师建议的，那么随便选择一个sp课程
        self._double_click(context, sp_best_course[0] + 80, sp_best_course[1] + 80)
        time.sleep(self.ACTION_DELAY)
        return True
    ...
    def _handle_low_health(self, context: Context, image, available_events: List[Dict]) -> bool:
        """处理低体力情况"""
        health_ratio = self._get_health_ratio(context, image)
        if health_ratio is None or health_ratio > max(self.LOW_HEALTH_THRESHOLD, self.GO_OUT_HEALTH_THRESHOLD):
            return False

        # 优先选择外出
        go_out_box = self._get_event_box("外出", available_events)
        if go_out_box and health_ratio <= self.GO_OUT_HEALTH_THRESHOLD:
            logger.info("体力过低，选择外出")
            self._double_click(context, go_out_box[0] + 80, go_out_box[1] + 80)
            time.sleep(self.ACTION_DELAY)
            return True
        ...
        if reco_detail.hit and health_ratio <= self.LOW_HEALTH_THRESHOLD:
